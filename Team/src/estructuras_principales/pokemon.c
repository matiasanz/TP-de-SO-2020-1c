...
//TAD pokemon
pokemon pokemon_create(especie_pokemon especie, t_posicion unaPosicion){
	return (pokemon) {especie, posicion_create(unaPosicion.pos_x, unaPosicion.pos_y)};
}

pokemon* pokemon_ptr_create(especie_pokemon especie, t_posicion unaPosicion){
	pokemon*unPokemon = malloc(sizeof(pokemon));
		   *unPokemon = pokemon_create(especie, unaPosicion);
    return unPokemon;
}
