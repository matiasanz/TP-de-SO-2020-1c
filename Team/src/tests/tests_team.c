...
context(tests_team){
	describe("posicion"){
		...
		it("Se creo una posicion"){
			should_int(unaPosicion.x) be equal to(1);
			should_int(unaPosicion.y) be equal to(0);
		}end
	...
	describe("Mapa"){ //cambiarle el nombre al tipo de dato
		...
		it("un pokemon se mapea correctamente"){
			pokemon*pokemonMapeado = list_get(mapaDePokemones, 0);
	...
	describe("Pendientes"){
		...
		it("un mensaje pendiente se agrega correctamente"){
			pendiente* agregado = list_remove(mensajesPendientes, 0);
		...
		it("un mensaje pendiente se obtiene a partir de su id"){
			pendiente* agregado = pendiente_get(mensajesPendientes, respuestaPendiente.id);
			assert_equals_pendiente(agregado, respuestaPendiente);
			pendiente_destroy(agregado);
		}end
		...
		it("un mensaje pendiente se quita correctamente"){
			pendiente* agregado = pendiente_get(mensajesPendientes, respuestaPendiente.id);
			list_is_empty(mensajesPendientes);
			pendiente_destroy(agregado);
		}end
				
		skip ("si se coloca un id incorrecto, corta la ejecucion"){
			pendiente_get(mensajesPendientes, 2500);
		}end
	}end
}
